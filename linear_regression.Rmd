...
### On The Issue of Overfitting

Although we have a model that is fairly good at predicting the price of a diamond, one issue comes up. That is, our model may be tuned too well to minimize the cost of the training set, but it may not generalize well for the test set. It is for that reason that the dataset can also be split such that there is also a cross-validation set. This cross-validation set is used to determine the value of $\lambda$ in what is known as *regularization*. Regularization is the process of trying to punish $\theta$ values that are too large and don't generalize the data well, and I may implement this some time soon.
